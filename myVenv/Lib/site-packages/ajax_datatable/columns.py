...
class Column(object):
    ...
    def search_in_choices(self, pattern_list):
        ...
        for pattern in pattern_list:
            ...
            values += [
                key
                for (key, text) in self._choices_lookup.items()
        ...
        for current_path_item in self._field_path:
            try:
                ...
                try:
                    ...
                    try:
                        current_value = [
                            getattr(f, current_path_item) for f in current_value
                        ]
                    except AttributeError:
                        current_value = None
...
class ManyToManyColumn(ForeignColumn):
    def get_foreign_value(self, obj):
        ...
        return [getattr(x, m2m_field) for x in eval(to_eval)]

    def render_column_value(self, obj, value_list):
        if self._allow_choices_lookup:
            return ", ".join(
                [str(self._choices_lookup.get(value, "")) for value in value_list]
            )
...
class PlaceholderColumnLink(ColumnLink):
    def __init__(self):
        super(PlaceholderColumnLink, self).__init__(None, None, False, False, True)
        ...
        try:
            ...
            if self.column_link.placeholder:
                raise ColumnOrderError(
                    "Requested to order a placeholder column (index %d)" % column_index
                )
        except KeyError:
            raise ColumnOrderError(
                "Requested to order a non-existing column (index %d)" % column_index
            )

    def __repr__(self):
        return "%s: %s" % (self.column_link.name, "ASC" if self.ascending else "DESC")
