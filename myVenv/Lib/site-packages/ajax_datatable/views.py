...
class AjaxDatatableView(View):
    ...
    def initialize(self, request):
            ...
            if c["name"]:
                ...
                for key in c.keys():
                    if key not in valid_keys:
                        raise Exception(
                            'Unexpected key "%s" for column "%s"' % (key, name)
                        )
        ...
        for cs in self.column_specs:
            ...
            #     - BooleanFields deserve a special treatement
                ...
                if len(choices) <= 0 and cs["autofilter"]:
                    choices = self.list_autofilter_choices(
                        request, cs, column.model_field, cs["initialSearchValue"]
                    )
        ...
        if show_column_filters is None:
            ...
            num_searchable_columns = len(
                [
                    c
                    for c in self.column_specs
                    if c.get("searchable") and c.get("visible")
                ]
            )
        ...
        if TRACE_COLUMNDEFS:
            trace(
                json.dumps(self.column_specs, indent=2, cls=DjangoJSONEncoder),
                prompt="column_specs",
            )
        ...
        for position, direction in initial_order:
            if type(position) == str:
                ...
                if position in keys:
                    ...
                    error_message = (
                        'Order key "%s" is invalid; available keys: "%s"'
                        % (position, ",".join(keys))
                    )
            ...
            if action == "initialize":
                ...
                for col in initial_order_columns:
                    if col >= len(self.column_specs):
                        ...
                        raise Exception(
                            'Column %d ("%s") is not orderable'
                            % (
                                col,
                                self.column_specs[col]["name"],
                            )
                        )
    ...
    def render_row_details(self, pk, request=None):
        ...
        relateds = []
        if (
            not self.disable_queryset_optimization_only
            and not self.disable_queryset_optimization_select_related
        ):
            relateds = [
                f.name
        ...
        if (
            not self.disable_queryset_optimization_only
            and not self.disable_queryset_optimization_prefetch_related
        ):
            prefetchs = [
                f.name
                for f in self.model._meta.get_fields()
                if f.many_to_many and f.concrete
            ]

        obj = (
            self.model.objects.filter(pk=pk)
            .select_related(*relateds)
            .prefetch_related(*prefetchs)
            .first()
        )
        ...
        try:
            template = loader.select_template(
                [
                    "ajax_datatable/%s/%s/%s"
                    % (
                        self.model._meta.app_label,
                        self.model._meta.model_name,
                        self.render_row_details_template_name,
                    ),
                    "ajax_datatable/%s/%s"
                    % (
                        self.model._meta.app_label,
    ...
    def render_row_tools_column_def():
        column_def = {
            ...
            "defaultContent": render_to_string(
                "ajax_datatable/row_tools.html", {"foo": "bar"}
            ),
    ...
    def get(self, request, *args, **kwargs):
        ...
        if TRACE_QUERYDICT:
            trace(
                json.dumps(query_dict, indent=2, cls=DjangoJSONEncoder),
                prompt="query_dict",
            )
        ...
        if not DISABLE_QUERYSET_OPTIMIZATION and not self.disable_queryset_optimization:
            if (
                ...
                and self.disable_queryset_optimization_only
                and self.disable_queryset_optimization_prefetch_related
            ):
                pass
            else:
        ...
        response_dict = self.get_response_dict(
            request, paginator, params["draw"], params["start"]
        )
        ...
        response = HttpResponse(
            json.dumps(response_dict, cls=DjangoJSONEncoder),
    ...
    def read_parameters(self, query_dict):
        ...
        """
        params = {
            field: int(query_dict[field]) for field in ["draw", "start", "length"]
        }
        ...
        while order_index < len(columns) and not has_finished:
            try:
                ...
                orders.append(
                    Order(order_column, query_dict[order_base + "[dir]"], column_links)
                )
        ...
        for column in self.column_specs:
            ...
            if foreign_field:
                ...
                if len(split_field) != 2:
                    raise Exception(
                        "m2m_foreign_field should be 2 level max ex : authors__name"
                    )
    ...
    def filter_queryset(self, params, qs):
        ...
        if self.show_date_filters:
            qs = self.filter_queryset_by_date_range(
                params.get("date_from", None), params.get("date_to", None), qs
            )
        ...
        for column_link in params["column_links"]:
            if column_link.searchable and column_link.search_value:
                qs = self.filter_queryset_by_column(
                    column_link.name, column_link.search_value, qs
                )
    ...
    def choice_field_search(self, column, search_value):
        ...
        # ]
        matching_choices = [
            val for key, val in values_dict.items() if key.startswith(search_value)
        ]
    ...
    def _filter_queryset(self, column_names, search_value, qs, global_filtering):
        ...
        if TEST_FILTERS:
            trace(
                ", ".join(column_names),
                prompt='%s filtering "%s" over fields'
                % ("Global" if global_filtering else "Column", search_value),
            )
        ...
        for column_name in column_names:
            ...
            column_filter = build_column_filter(
                column_name, column_obj, column_spec, search_value, global_filtering
            )
            if column_filter:
                ...
                if TEST_FILTERS:
                    try:
                        ...
                        trace(
                            '%8d/%8d records filtered over column "%s"'
                            % (
                                qstest.count(),
                                qs.count(),
                                column_name,
                        ...
                        trace(
                            'ERROR filtering over column "%s": %s'
                            % (column_name, str(e))
                        )
    ...
    def list_autofilter_choices(
        self, request, column_spec, field, initial_search_value
    ):
        """
