...
class BaseAdapterRegistryTests(unittest.TestCase):
        ...
        class _CUT(BaseAdapterRegistry):
            class LookupClass:
                ...
                def remove_extendor(self, provided):
                    self._extendors = tuple(
                        [x for x in self._extendors if x != provided]
                    )
    ...
    def test_register(self):
        ...
        self.assertEqual(
            registered,
            [((IB0,), IR0, "", "A1")],  # required  # provided  # name  # value
        )
    ...
    def test_register_multiple_allRegistrations(self):
        ...
        self.assertEqual(
            registry._adapters,
            build_adapters(L=self._getMutableListType(), MT=self._getMappingType()),
        )
        ...
        self.assertEqual(
            registry._adapters, build_adapters(L=CustomSequence, MT=CustomMapping)
        )
    ...
    def test_register_with_same_value(self):
        ...
        MT = self._getMappingType()
        self.assertEqual(
            registry._adapters[1], MT({Interface: MT({IR0: MT({"": _value})})})
        )
        registered = list(registry.allRegistrations())
        self.assertEqual(
            registered,
            [((Interface,), IR0, "", _value)],  # required  # provided  # name  # value
        )
    ...
    def test_unsubscribe_hit(self):
        ...
        self.assertEqual(
            registry._subscribers[1], MT({IB1: MT({None: MT({"": L((orig,))})})})
        )
    ...
    def test_subscribe_multiple_allRegistrations(self):
        ...
        self.assertEqual(
            registry._subscribers,
            build_subscribers(L=CustomSequence, F=CustomLeafSequence, MT=CustomMapping),
        )
...
class VerifyingBaseFallbackTests(unittest.TestCase):
    ...
    def _makeOne(
        self, registry, uc_lookup=None, uc_lookupAll=None, uc_subscriptions=None
    ):
        # pylint:disable=function-redefined
        if uc_lookup is None:
