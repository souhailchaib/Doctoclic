...
class Declaration(Specification):
    ...
    def _add_interfaces_to_cls(interfaces, cls):
        ...
        interfaces = tuple(
            [iface for iface in interfaces if not implemented_by_cls.isOrExtends(iface)]
        )
...
class Implements(NameAndModuleComparisonMixin, Declaration):
    ...
    def __repr__(self):
        if self.inherit:
            name = getattr(self.inherit, "__name__", None) or _implements_name(
                self.inherit
            )
        else:
            name = self.__name__
...
def moduleProvides(*interfaces):
    ...
    p = locals["__provides__"] = Provides(ModuleType, *_normalizeargs(interfaces))
