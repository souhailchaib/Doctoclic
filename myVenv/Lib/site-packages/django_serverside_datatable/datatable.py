...
class DataTablesServer(object):
    ...
    def run_queries(self):
        ...
        if _filter:
            if op == "or":
                ...
                data = qs.filter(reduce(operator.and_, _filter)).order_by(
                    "%s" % sorting
                )
            len_data = data.count()
    ...
    def filtering(self):
        ...
        or_filter = []
        op = ""
        if (self.request_values.get("sSearch")) and (
            ...
            op = "and"
            for i in range(len(self.columns)):
                if (self.request_values.get(f"sSearch_{i}")) and (
                    self.request_values[f"sSearch_{i}"] != ""
                ):
                    or_filter.append(
                        (
                            self.columns[i] + "__icontains",
                            self.request_values[f"sSearch_{i}"],
                        )
                    )
