    ...
    def setUp(self):
        ...
        for fbs_file in ["wamp.bfbs", "testsvc1.bfbs"]:
            archive = pkg_resources.resource_filename(
                "autobahn", "xbr/test/catalog/schema/{}".format(fbs_file)
            )
...
class TestFbsRepository(TestFbsBase):
    ...
    def test_loaded_schema(self):
        schema_fn = pkg_resources.resource_filename(
        ...
        call: FbsRPCCall = schema.services["testsvc1.ITestService1"].calls[
            "run_something1"
        ]
        ...
        call_type: FbsObject
        for call_type in [
            schema.objs[call.request.name],
            schema.objs[call.response.name],
        ]:
                    ...
                    for union_type_value in call_type_enum.values:
                        if union_type_value != "NONE":
                            ...
                            if "." in call_type_enum.name:
                                ...
                                union_type_qn = "{}.{}".format(
                                    namespace, union_type_value
                                )
                            else:
...
class TestFbsValidateEthAddress(TestFbsBase):
    def test_validate_obj_EthAddress_valid(self):
        ...
        for value in [
            ...
            {
                "value": pack_ethadr(
                    "0x0000000000000000000000000000000000000000", return_dict=True
                )
            },
            {
                "value": pack_ethadr(
                    "0xecdb40C2B34f3bA162C413CC53BA3ca99ff8A047", return_dict=True
                )
            },
        ]:
            try:
                ...
                self.assertTrue(
                    False, f"Inventory.validate() raised an exception: {exc}"
                )
