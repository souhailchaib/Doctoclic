...
class EventConsumer(AsyncWebsocketConsumer):
    ...
    async def create_event(self, data):
        ...
        end = datetime.datetime.strptime(data.get('end', ''), '%Y-%m-%dT%H:%M:%S.%fZ') if data.get('end') else None
    ...
    async def update_event(self, data):
        ...
        try:
            ...
            await self.send(text_data=json.dumps({'message': 'Event updated successfully.'}))
    ...
    async def delete_event(self, data):
        ...
        try:
            ...
            # Send a response back to the client if needed
            await self.send(text_data=json.dumps({'message': 'Event deleted successfully.'}))
